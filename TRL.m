...
% chose the sign of a 
  ...
  if abs(angle(R1a)*180/pi)>90 % since we have a short it is >90 if it was open it would be <90
    a = a;
    as(n) = a;
    c = ca*a;
  end;

  if abs(angle(R1b)*180/pi)>90 % since we have a short it is >90 if it was open it would be <90
    a = -a;
    as(n) = a;
    c = ca*a;
  end;
